...
function ViewCourse() {
  ...
  const handleEnroll = async(userId, courseId)=>{
    try{
      const orderData = await axios.post(serverUrl + "/api/payment/razorpay-order", {userId, courseId}, {withCredentials:true})
      ...
      const options ={
        ...
        handler: async function (response){
          ...
          try{
            const verifyPayment = await axios.post(serverUrl + "/api/payment/verify-payment", {...response, courseId, userId}, {withCredentials:true})
            setIsEnrolled(true);
  ...
  const handleReview = async()=>{
    setLoading(true);
    try{
      const result = await axios.post(serverUrl + "/api/review/createrview", {courseId, rating, comment}, {withCredentials:true})
      setLoading(false);
  ...
  return (
      ...
      <div className='flex flex-col md:flex-row gap-6'>
        ...
        <div className='flex flex-col gap-3'>
          {selectedCourse?.lectures?.map((lecture, index)=>(
            <button key={index} disabled={!lecture.isPreviewFree} onClick={()=>{if(lecture.isPreviewFree){setSelectedLecture(lecture)}}} className={`flex items-center gap-3 px-4 py-3 rounded-lg border transition-all duration-200 text-left ${lecture.isPreviewFree? "hover:bg-gray-100 cursor-pointer border-gray-300" : "cursor-not-allowed opacity-60 border-gray-200"} ${selectedCourse?.lectureTitle === lecture?.lectureTitle ? "bg-gray-100 border-gray-400" : ""}`}>
    ...
    <div className='mt-8 border-t pt-6'>
      ...
      <div className='mb-4'>
        ...
        <textarea onChange={(e)=>setComment(e.target.value)} value={comment} className='w-full border border-gray-300 rounded-lg p-2' placeholder='Write your review here... ' 
        rows={3}
        />
