    ...
    const handleRecommendation = async (query) => {
        ...
        try{
            ...
            if(result.data.length > 0){
                speak("These are the courses I found for you")
            }
            else{
                speak("Sorry, I couldn't find any courses matching your query")
            }
        }catch(error){
            console.log(error)
            setListening(false)
    ...
    <div className='min-h-screen bg-gradient-to-br from-black to-gray-900 text-white flex flex-col items-center px-4 py-1'>
        ...
        <div className='bg-white shadow-xl rounded-3xl p-6 sm:p-8 w-full max-w-2xl text-center relative'>
            ...
            <div className='flex items-center bg-gray-700 rounded-full overflow-hidden shadow-lg relative w-full'>
                <input type="text" className='flex-grow px-4 py-3 bg-transparent text-white placeholder-gray-400 focus:outline-none text-sm sm:text-base' placeholder='What do you want to learn? (e.g. AI, MERN, CLOUD....)' onChange={(e)=>setInput(e.target.value)} value={input}/>
        ...
        {recommendations.length > 0 ? (
            <div className='w-full max-w-6xl mt-12 px-2 sm:px-4'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8'>
